...
function renderCategoryChart(catStats, categories) {
  ...
  if (emptyEl) emptyEl.classList.add('hidden');
  if (!isChartAvailable()) {
    renderSimpleBars(canvas, monthlyData.map(m => ({ label: m.label, value: m.total })), emptyEl, DB.getSettings().currency || 'R$');
    return;
  }
  hideFallbackBox(canvas);
...
function renderMonthlyChart(monthlyData) {
  ...
  if (!isChartAvailable()) {
    renderSimpleBars(canvas, monthlyData.map(m => ({ label: m.label, value: m.total })), null, cur);
    return;
  }
