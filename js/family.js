...
function generateShareQR() {
  ...
  if (typeof window.QRCode === 'undefined') {
    container.innerHTML = '<div class="qr-fallback-box"><strong>Modo offline:</strong><p>O QR visual não está disponível neste dispositivo, mas os dados já podem ser copiados e enviados.</p></div>';
    showToast('Modo offline: use o botão copiar dados para compartilhar.', 'info', 4500);
  } else {
    try {
      if (qrInstance) { try { qrInstance.clear(); } catch(e) { console.debug('Falha ao limpar QR anterior', e); } }
      qrInstance = new QRCode(container, {
        text: 'MS:' + encoded,
      ...
      container.innerHTML = '<div class="qr-fallback-box"><strong>Modo offline:</strong><p>Não foi possível desenhar o QR Code. Use a cópia dos dados.</p></div>';
      console.error('Erro ao gerar QR Code', e);
      showToast('Erro ao gerar QR Code visual. Use a cópia dos dados.', 'warning');
    }
  }
...
function startQRDecode(video) {
  if (typeof window.jsQR === 'undefined') {
    showToast('Leitor visual de QR indisponível neste dispositivo. Use a importação por colar dados.', 'warning', 5000);
    stopQRScan();
    return;
  }
