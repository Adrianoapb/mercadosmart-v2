...
window.Auth = (() => {
  ...
  function loadState() {
    ...
    } catch (e) { console.debug('Sessão local inválida, limpando armazenamento', e); }
  }
  ...
  async function bootstrap() {
    ...
    try {
      ...
      if (state.user) {
        ...
        applyUserContext();
        hideAuthScreen();
        showToast('Modo offline ativo com a última sessão salva.', 'warning', 4500);
        return true;
      }
      clearState();
      showAuthScreen('login');
  ...
  async function logout() {
    try { await api('logout'); } catch (e) { console.debug('Logout remoto indisponível, seguindo logout local', e); }
    clearState();
