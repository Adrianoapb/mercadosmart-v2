...
function requireAuthenticatedCloudAction() {
  if (window.Auth && typeof Auth.isAuthenticated === 'function' && Auth.isAuthenticated()) return true;
  const detailEl = document.getElementById('cloudStatusDetail');
  if (detailEl) detailEl.textContent = 'Faça login para usar a nuvem com segurança.';
  DB.CloudSync.setMeta({ status: 'error', lastError: 'Faça login para sincronizar.' });
  renderCloudStatus(DB.getCloudMeta());
  document.getElementById('authOverlay')?.classList.remove('hidden');
  document.getElementById('app')?.classList.add('hidden');
  if (window.Auth && typeof Auth.switchMode === 'function') Auth.switchMode('login');
  showToast('Faça login para sincronizar.', 'warning', 4500);
  return false;
}
...
function loadCloudSettings() {
  ...
  const accountName = document.getElementById('cloudAccountName');
  if (accountName) accountName.textContent = (window.Auth && typeof Auth.getUser === 'function' && Auth.getUser()?.name) || profile.name || 'Conta protegida';
  ...
  if (accountHint) {
    ...
    accountHint.textContent = user ? `E-mail: ${user.email || '-'} • Sessão ativa e sincronização individual liberada` : 'Faça login para liberar sincronização e backup.';
  }
...
function saveCloudProfileSettings() {
  showToast('Esta versão usa a conta autenticada para sincronizar. O workspace manual foi removido.', 'info', 4500);
}


async function testCloudConnection() {
  if (!requireAuthenticatedCloudAction()) return;
  try {
