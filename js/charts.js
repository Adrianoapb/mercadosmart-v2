...
function renderMonthlyChart(monthlyData) {
  ...
  if (!canvas) return;
  hideFallbackBox(canvas);
  if (monthlyChartInstance) { try { monthlyChartInstance.destroy(); } catch(e) {} monthlyChartInstance = null; }

  const hasData = Array.isArray(monthlyData) && monthlyData.some(m => (m.total || 0) > 0);
  if (!hasData) {
    if (emptyEl) emptyEl.classList.remove('hidden');
    canvas.style.display = 'none';
    return;
...
function renderRepMonthlyLine(monthlyData) {
  ...
  if (repMonthlyLineInstance) { try { repMonthlyLineInstance.destroy(); } catch(e) {} repMonthlyLineInstance = null; }
